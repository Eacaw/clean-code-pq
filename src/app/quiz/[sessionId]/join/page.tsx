    ...
    async function fetchSessionData() {
      try {
        ...
        if (data.status !== "active") {
          setError("This session is not currently active. Please try again later.")
        }
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="bg-gray-900 rounded-lg shadow-lg p-6">
        ...
        {error ? (
          <div className="bg-red-900/30 text-red-400 p-4 rounded-md mb-6">{error}</div>
        ) : (
          <div className="mb-6">
            <h2 className="text-xl font-semibold mb-2">{sessionData?.name || "DevDay Session"}</h2>
            <p className="text-gray-400">Enter your team name to join this coding competition.</p>
          </div>
        )}
